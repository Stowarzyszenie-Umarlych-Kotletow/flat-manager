...
export function TaskDetailsModal({ setShow, taskId, taskInstance, deletable = false }:
  ...
  return (
    ...
    >
		<ScrollView>
		<ModalContent>
			<Text style={styles.tinyTextCenter}>User assigned to task at {sliceDate(taskInstance.date)} by {getUsername(taskInstance.userId)}</Text>
			<Text style={styles.tinyTextCenter}>Task </Text>
			<Text style={styles.smallTextCenter}>Users in periodic task {task.name}:</Text>
			<ul>
